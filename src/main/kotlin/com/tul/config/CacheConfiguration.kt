...
class CacheConfiguration(    @Autowired val gitProperties: GitProperties?,
    ...
    fun cacheManagerCustomizer(): JCacheManagerCustomizer {
        return JCacheManagerCustomizer { cm ->
            createCache(cm, com.tul.repository.UserRepository.USERS_BY_LOGIN_CACHE)
            createCache(cm, com.tul.repository.UserRepository.USERS_BY_EMAIL_CACHE)
            createCache(cm, com.tul.domain.User::class.java.name)
            createCache(cm, com.tul.domain.Authority::class.java.name)
            createCache(cm, com.tul.domain.User::class.java.name + ".authorities")
